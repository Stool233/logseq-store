- JIT编译器（Just-in-Time Compilation）是一种特殊类型的编译器，它在程序运行时将高级语言或字节码转换成机器码，目的是提高程序的执行效率并减少编译前的等待时间。与AOT（Ahead-of-Time）编译器（ [[AOT编译器]] ）不同，AOT在程序运行前就完成了编译过程，而JIT编译过程是在程序实际运行时进行。
- ### 工作原理
