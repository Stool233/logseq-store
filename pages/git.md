...
- git-subtrac
	...
	- git submodules存在的一些问题
		- 自Git最早的日子以来，Git子模块一直非常复杂且难以使用，而且没有太大的改进。这些复杂性源于一些主要的设计问题：
			- 子模块的内容来自其他Git仓库，这些仓库可能会在没有警告的情况下移动位置，或者被rebase、删除等，导致您自己的仓库不再可用。至少，使用多个上游仓库使得分叉项目并进行广泛更改变得困难：您必须为您更改的每一个子仓库分叉，然后更新.gitmodules链接，告诉所有人如何获取您的更改等。结果是，子模块迫使人们集中在单个上游仓库上。