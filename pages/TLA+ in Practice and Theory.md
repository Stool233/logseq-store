...
- https://pron.github.io/posts/tlaplus_part4
	...
	- ### 模块
		...
		- 首先，我们构建一个三维向量，记为
		   $\triangleq \text{ConsVector}(\langle 4.5, 1.6, -18.99 \rangle$
		  由于编码对我们来说是隐藏的（换句话说，`Vector3D` 没有导出任何定理说明它的向量是集合 `[x:S, y:S, z:S]` 中的记录），我们无法使用 `v.x`。此外，因为我们已经将 `ConsVector` 和 `Elem` 的定义隐藏在 `LOCAL` 操作符内，所以这些定义的细节无法通过带有 `BYDEF` 的证明进行检查。
		- 如果一个模块 `Foo` 包含了假设 `ASSUMEA`（对于某些假设 `A`）和一个定理 `THEOREMT`，并且用 `MyFoo \triangleq INSTANCE Foo WITH …` 实例化，那么 `MyFoo!T` 将引用定理 `¯¯¯¯APROVE¯¯¯¯T`（记住，`¯¯¯¯A` 和 `¯¯¯¯T` 表示在 `WITH` 子句中提供的替代后的 `A` 和 `T`）。换句话说，当实例化时，模块中的所有假设都被添加为其定理的假设。这是因为模块中的定理依赖于模块中的所有假设，它们将这些假设视为公理。
