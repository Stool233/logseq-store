...
- https://pron.github.io/posts/tlaplus_part4
	...
	- ### 模块
		...
		- 这非常类似于（非法的）`MyM2 ≜ INSTANCE M2(M1)`，假设`M1`和`M2`的“签名”匹配，即`M1`导出了`M2`声明所需的名称。
		- 实际上，模块实例——就像我们在第二部分看到的运算符元数和第三部分看到的TLA“层级”——是TLA+在语法层面使用类似于推断类型的另一个例子。这种语言跟踪哪些定义（顶级或在`LET/IN`块中）是模块实例，并且如果需要使用另一个时使用其中一个是语法错误。此外，语法不仅跟踪一个符号是否引用模块实例，如果引用了，它还跟踪是哪个模块，因此符号解析从不会模糊（不像大多数无类型编程语言中的情况）。可以公平地说，TLA+的模块系统实际上是以一种类似于ML中的模块的方式进行类型化的。
		- 模块可以用来抽象掉数学对象的编码（“实现”）细节。这里有一个（非常）人为的例子，它抽象地定义了向量，而没有指定任何特定的编码：
		-
		-