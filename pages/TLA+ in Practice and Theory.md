...
- https://pron.github.io/posts/tlaplus_part4
	...
	- ## **组合**
		...
		- ### 基于模型的规范和信息隐藏
			...
			- 这个堆栈规范是代数规范（*algebraic* specification）的一个示例，而代数规范又是公理化规范（*axiomatic* specification）的一个特例。公理化规范指定了某个系统或数据结构的外部行为——其接口——作为它必须满足的一系列属性。这种方法乍看之下非常优雅，非常抽象，非常数学化。但试试看是否可以用这种方式指定一个 FIFO 队列；或者一个 B-树；或者一个并发 B-树。对于非逻辑学家来说，弄清楚如何将一个系统指定为一系列公理是非常困难的。例如，使用 `Least` 是必要的，以排除无限堆栈，这对非逻辑学家来说可能不是显而易见的。
