...
- https://pron.github.io/posts/tlaplus_part4
	...
	- ### 替代性的操作符
		- 在给常量声明赋予时间意义（涉及时间变量的表达式）的技术问题中，存在一种特殊情况。一个常量模块是指所有声明都是常量的模块（即，它不声明任何时间变量），并且其所有定义都是常量级别的，意味着它们不使用素数操作符或任何时间操作符。如果一个模块 `M` 不是常量模块，其常量声明只能被赋予常量意义——不是变量、状态函数或时间公式——并且其操作符声明必须被赋予常量级别的意义。然而，如果一个模块是常量模块，那么其常量可以被赋予非常量的意义，前提是其操作符声明被赋予的意义是莱布尼茨的或替代的，我将解释这一点。考虑以下模块：
		-
		-
		-
		-