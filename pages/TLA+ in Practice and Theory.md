		...
		- ### 安全性与活性
			...
			- 如果 $\sigma$ 是某个行为，我们将让 $\sigma^n$ 表示行为的长度为 $n$ 的前缀。如果我们有一个无限的行为序列，$\sigma_1, \sigma_2, \ldots$ 我们将说行为 $\sigma$ 是它们的极限，即 $\sigma = \lim_{i \to \infty} \sigma_i$（ 或者只是 $\sigma = \lim \sigma_i$），当且仅当 直观上，行为的前缀收敛于 $\sigma$。
				- 更精确地说，$\sigma = \lim \sigma_i$  当且仅当 对于每个 $n \geq 0$， 存在 $m \geq 0$ 使得 $i > m \Rightarrow \sigma_i^n = \sigma^n$。
					- 换句话说，存在某个点 $m$ 之后，所有行为序列的前 $n$ 个元素都与 $\sigma$ 的前 $n$ 个元素相同。
					- 让我们用一个简单的例子来说明这些定义：
						- 设 $\sigma = a, b, c, d, e, \ldots$ 是一个无限序列。
						- 我们定义行为序列 $\sigma_1, \sigma_2, \ldots$ 如下：
						- $\sigma_1 = a, b, c, d, e, \ldots$
						- $\sigma_2 = a, b, c, d, e, \ldots$
						- $\sigma_3 = a, b, c, d, e, \ldots$
						- ...
						- 每个 $\sigma_i$ 都与 $\sigma$ 完全相同。
						- 对于每个 $n \geq 0$，行为 $\sigma$ 的前缀 $\sigma^n$ 为 $a_1, a_2, \ldots, a_n$。对于每个 $i$，行为 $\sigma_i$ 的前缀 $\sigma_i^n$ 也是 $a_1, a_2, \ldots, a_n$。
						- 无论 $n$ 取何值，$\sigma_i^n$ 都等于 $\sigma^n$，并且这一等式对于所有 $i \geq 1$ 都成立。因此，存在 $m = 0$ 使得对于所有 $i > m$，$\sigma_i^n = \sigma^n$。即：
						- $\sigma = \lim_{i \to \infty} \sigma_i$
						- 满足条件：对于每个 $n \geq 0$，存在 $m \geq 0$ 使得 $i > m \Rightarrow \sigma_i^n = \sigma^n$。
