	...
	- ## **综合起来**
		...
		- ### 数据细化和归纳数据类型
			...
			- ```
			  ...
			  ListAsSeq(ll) ≜ LET RECURSIVE View(_, _) View(i, node) ≜ IF i = 1 THEN node.value ELSE View(i-1, node.next) IN [i ∈ 1..LenList(ll) ↦ View(i, ll)]
			  RECURSIVE SeqAsList(_) SeqAsList(s) ≜ IF s = ⟨⟩ THEN EMPTY ELSE [value ↦ s[1], next ↦ SeqAsList(Tail(s))]
			  ```
