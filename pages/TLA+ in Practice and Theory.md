...
- https://pron.github.io/posts/tlaplus_part4
	...
	- ## **其他等价和顺序的概念**
		...
		- ### 伽罗瓦连接（Galois Connections）
			...
			- 如果 \( \mathcal{P} = \langle P, \preceq \rangle \) 和 \( \mathcal{Q} = \langle Q, \sqsubseteq \rangle \) 是偏序集（部分有序集），并且 \( f_* : P \rightarrow Q \), \( f^* : Q \rightarrow P \) 是函数，那么对 \( \langle f^*, f_* \rangle \) 是一个伽罗瓦连接，如果对所有 \( p \in P \), \( q \in Q \) 有：\( f^*(p) \sqsubseteq q \) 当且仅当 \( p \preceq f_*(q) \)。然后 \( f^* \) 被称为下伴随（连接的左伴随），\( f_* \) 被称为上伴随（连接的右伴随）。\( f^* \) 和 \( f_* \) 互相唯一确定；设置一个强制设定另一个。
