	...
	- ## **逻辑公式和表达式**
		...
		- ### 自由变量和未解释变量：CONSTANT
			...
			- 在 TLA+ 中，自由变量/未解释符号使用关键字 \(\text{CONSTANT}\)（或其同义词 \(\text{CONSTANTS}\)）声明，关于它们的假设 — 即公理性地断言为真的公式 — 使用关键字 \(\text{ASSUME}\)（或其同义词 \(\text{AXIOM}\)）引入。例如，如果 \(P\) 和 \(Q\) 是一些谓词，我们可以写：
				- ![image.png](../assets/image_1715700473572_0.png){:height 117, :width 234}
			...
			- 虽然 \(\text{CHOOSE } x: P(x)\) 和
				- ![image.png](../assets/image_1715783392876_0.png){:height 87, :width 259}
			...
			- \[
			  ...
			  \]
			- 这样的表达方式明确指出，我们认为 \(F\) 和 \(G\) 在所有可能的 \(x\) 值上的行为是相同的，从而避免了直接比较两个操作符的语法和逻辑问题。在 TLA+ 中，这种处理方法确保了表达式的严谨性和清晰性，同时允许我们在逻辑上操作和推理关于操作符的性质和行为。
