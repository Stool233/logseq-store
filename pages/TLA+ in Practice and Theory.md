	...
	- ## **集合**
		...
		- ### 记录（Records）
			...
			- $$
			  ...
			  $$
			- 并且 \( r.a \) 是 \( r['a'] \) 的语法糖。EXCEPT 也有专门针对记录的特殊语法，所以一个等于 \( r \) 但其“字段” \( b \) 是 200 的记录，可以用 $$[r \ \text{EXCEPT} !.b = 200]$$（或者用$$[r \ \text{EXCEPT} !.b = @ \times 10]$$）来创建。嵌套结构也同样适用（例如，
			- $$
			  [r \ \text{EXCEPT} !.a.k = @ + 1]
			  $$
			- ）。最后，我们有专门针对记录集的特殊语法：
			- $$
			  [\text{name: STRING}, \text{id: Nat}]
			  $$
