...
- https://pron.github.io/posts/tlaplus_part2
	...
	- ## **集合**
		...
		- ### 序列和元组
			- 序列是一个有限或无限的值的有序列表。在 TLA+ 中，序列被定义为某个前缀 \( \text{Nat} \setminus \{0\} \) 上的函数，因此使用从1开始的索引，这是数学中的通常做法，但在大多数编程语言中不常见，那里通常使用从0开始的索引。
			- 所有偶数的升序序列可以表示为\[ [n \in \text{Nat} \setminus \{0\} \mapsto 2 \times (n-1)] \]，而介于2到200之间的所有偶数的序列为\[ [n \in 1..100 \mapsto 2 \times n] \]。
			- Sequences 模块包含了几个用于处理序列的实用定义。\( \text{Seq}(S) \) 是集合 \( S \) 上所有有限序列的集合（无限序列的集合则为 \( [\text{Nat} \setminus \{0\} \rightarrow S] \)），定义如下：
			- $$\text{Seq}(S) \triangleq \text{UNION} \{ [1..n \rightarrow S] : n \in \text{Nat} \} $$
