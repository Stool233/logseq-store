	...
	- ## **标准模型、安全性与活性（The Standard Model, Safety and Liveness）**
		...
		- ### 安全性与活性
			...
			- 这个极限的定义允许我们将行为的宇宙转化为一个拓扑空间，
				- 通过定义一组行为 $S$ 为闭合（*closed*）的，当且仅当 它包含了所有的极限点。
				- 然后我们定义任何行为集 $S$ 的闭包（*closure*），记为 $\overline{S}$，为 $S$ 的所有极限点的集合。
				- $\overline{S}$ 是 $S$ 的最小闭合超集（smallest closed superset），而 $S$ 是闭合（closed）的，当且仅当 $S = \overline{S}$。
				- 我们将说一组行为 $S$ 是密集（*dense*）的，当且仅当 在空间中的任何行为都是 $S$ 的极限点。
