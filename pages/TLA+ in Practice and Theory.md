...
- https://pron.github.io/posts/tlaplus_part2
	...
	- ## **集合**
		...
		- ### 函数
			...
			- $$
			  g \bullet f \triangleq [x \in \{z \in DOMAIN \ f : f[z] \in DOMAIN \ g\} \mapsto g[f[x]]]
			  $$
			- 这样的定义确保了函数复合 \(g \bullet f\) 在 \(f\) 的输出属于 \(g\) 的定义域的前提下才进行操作，模拟了部分函数的行为。这是在数学中处理可能不存在的映射输出的一种方式，而在计算机科学中，这种情况通常通过异常处理或返回特殊值（如 null 或 None）来处理。
