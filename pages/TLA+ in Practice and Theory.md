...
- https://pron.github.io/posts/tlaplus_part2
	...
	- ## **集合**
		...
		- ### 集合基础
			...
			- 上述操作基于ZFC公理定义，充分指定了我们宇宙中存在哪些集合。由于 \(\text{CHOOSE}\) 只是选择值，你不能 \(\text{CHOOSE}\) 罗素悖论“集合”，因为这样的集合不存在，它不能使用任何集合构造操作来构建。因此，\(CHOOSE x: \forall s: s \in x \equiv s \notin s\)（这表明 \(x\) 是所有不包含自身的集合的集合——一个悖论）的值是未定义的，因为没有这样的集合 \(x\) 存在；\(CHOOSE\) 表达式的右侧对于ZFC中的所有集合来说都是假的。
			- 这就是为什么罗素悖论在ZFC公理系统中不构成问题的原因。ZFC通过限制集合构造的方式，有效地避免了这种自我引用和悖论的发生。这些操作和公理保证了集合理论的一致性和有用性，允许数学家和逻辑学家在坚实的基础上进行推理和建模。
