...
- https://pron.github.io/posts/tlaplus_part3
	...
	- ## **状态机**
		...
		- ### 非常简单的状态机
			...
			- 假设 `x` 是一个时态变量。表达“如果 x 是偶数，就将其除以二”的动作，可以写作 `x%2 = 0 ∧ x' = x/2`。这是一个常见的模式。注意，它与动作 `x%2 = 0 ⇒ x' = x/2` 不同。原因在于动作是对两个状态的谓词。在第一个动作中，当且仅当 `x` 是偶数并且 `x' = x/2` 时，公式才为真——即，加撇号的状态是未加撇号的状态的下一个状态。在第二个动作中，如果 `x` 是偶数并且 `x' = x/2`，那么谓词确实为真，但如果 `x` 不是偶数并且 `x'` 是任何值，因为当其左操作数为假时，含义就是真的；这很少是我们想要的。
