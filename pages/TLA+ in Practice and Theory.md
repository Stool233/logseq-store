...
- https://pron.github.io/posts/tlaplus_part3
	...
	- ## **标准模型、安全性与活跃性**
		...
		- ### 安全性与活性
			...
			- 如果 σ 是某个行为，我们将让 σ^n 表示行为的长度为 n 的前缀。如果我们有一个无限序列的行为，σ1, σ2, … 我们将说行为 σ 是它们的极限，即 σ=limi→∞σi 或只是 σ=limσi，如果直观上，行为的前缀收敛于 σ。更精确地说，σ=limσi 如果对于每个 n≥0 存在 m≥0 使得 i>m⇒σni=σn。如果 S 是一组行为，我们说行为 σ 是 S 的极限点，如果存在元素 σ1, σ2, … 在 S 中使得 σ=limσi。
			- 这个极限的定义允许我们将行为的宇宙转化为一个拓扑空间，通过定义一组行为 S 为闭合的，如果它包含了所有的极限点。然后我们定义任何行为集 S 的闭包，记为 ¯¯¯¯S，为 S 的所有极限点的集合。¯¯¯¯S 是 S 的最小闭合超集，而 S 是闭合的当且仅当 S=¯S。我们将说一组行为 S 是密集的，如果在空间中的任何行为都是 S 的极限点。