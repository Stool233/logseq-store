...
- https://pron.github.io/posts/tlaplus_part3
	...
	- ## **标准模型、安全性与活性（The Standard Model, Safety and Liveness）**
		...
		- ### 时态公式
			...
			- 我将我们逻辑中的灵活变量称为时态变量，并且与刚性变量类似（用关键词 `CONSTANT` 声明或通过量词 `∀` 和 `∃` 引入），时态变量是用关键词 `VARIABLE`（或其同义词 `VARIABLES`）声明的，或者通过时态量词引入的。一个时态变量在计算的每个状态可能有不同的值，即它可能在单个行为中改变——算法的执行。然而，需要记住的关键是，尽管 `CONSTANT` 在行为过程中不会改变，它们在不同的行为中可能是不同的。
