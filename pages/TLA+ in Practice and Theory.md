	...
	- ## **标准模型、安全性与活性（The Standard Model, Safety and Liveness）**
		...
		- ### 时态公式
			...
			- 现在事情变得更有趣了。在TLA中，我们可以编写一个表达式，这不是单一状态的函数，而是两个状态的函数。
				...
				- 如果表达式 `e` 是一个状态函数（或状态谓词），那么 `e'` 等于表达式 `e`，其中所有时态变量——以及它引用的定义——都被标记了。因此，状态函数 `e'` 的值是状态函数 `e` 在下一个状态中的值。包含标记变量或标记子表达式的表达式被称为转换函数或动作表达式，或者，如果它是谓词，则称为转换谓词（第2级）。转换谓词更广为人知的名称是动作，正是这些动作赋予了TLA其名称并构成了其核心。
