...
- https://pron.github.io/posts/tlaplus_part2
	...
	- ## **证明（Proofs）**
		...
		- 虽然 TLAPS 被设计为完全与后端无关，但在使用它时，你可能有时会发现有必要使用特定的求解器功能。例如，$BY Z3$ 可以告诉 TLAPS 使用 Z3 SMT 求解器来处理（验证）特定的证明，你甚至可以（虽然不推荐，而且在实践中你不太可能需要它）使用 Isabelle 策略。实际上，上面提供的 $atInduction$ 定理的证明是 $\text{BY} ~ IsaM('' (intro ~ natInduct, auto) '')$。
