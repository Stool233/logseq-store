...
- https://pron.github.io/posts/tlaplus_part4
	...
	- ## **组合**
		...
		- ### 基于模型的规范和信息隐藏
			...
			- 在第三部分中，我们简要地遇到了存在性时间量化 \( \bm{\exists} x : F \)，它表示“存在一个时间变量 \( x \)，它在每个状态下可能取不同的值，使得 \( F \)”成立。然而，将 \( \bm{\exists} x : F \) 看作是“\( F \) 中隐藏了 \( x \)”更为简单。这与一阶逻辑中的常规存在量化相似，例如，\( \bm{\exists} y \in Int : y * 2 = x \) 使用隐藏变量 \( y \) 来描述自由变量 \( x \)。绑定变量 \( y \) 不再是模型的一部分，模型仅描述自由变量 \( x \)。时间量化亦是如此。
			- 注意，在 `Stack ≜ ∃ buf : Internal(buf) ! IStack` 中隐藏 `buf` 必须在不在 `VARIABLES` 子句中声明 `buf` 为自由变量的不同模块中完成，因为 TLA+ 不允许重新定义任何名称。
		- **开放系统规范**
