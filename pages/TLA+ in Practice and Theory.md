...
- https://pron.github.io/posts/tlaplus_part2
	...
	- ## **集合**
		...
		- ### 集合基础
			...
			- 如果 \(S\) 是一个集合，那么 \(\text{SUBSET} ~ S\) 是 \(S\) 的幂集，即 \(S\) 的所有子集的集合（幂集公理），或者 \(\forall x: x \in \text{SUBSET} ~ S \equiv \forall z \in x: z \in S\)（注意对于任何集合 \(S\)，空集是 \(\text{SUBSET} ~ S\) 的成员，因为 \(\forall z \in \{\}: z \in S\)）。例如，\(\text{SUBSET} \{1, 2, 3\} = \{\{\}, \{1\}, \{2\}, \{3\}, \{1, 2\}, \{1, 3\}, \{2, 3\}, \{1, 2, 3\}\}\)。
