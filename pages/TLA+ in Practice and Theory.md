...
- https://pron.github.io/posts/tlaplus_part3
	...
	- ## **标准模型、安全性与活性（The Standard Model, Safety and Liveness）**
		...
		- ### 标准模型
			- 我们定义计算 —— 一种算法的执行 —— 为一种行为（*behavior*）。行为是状态的无限序列，而状态是变量对值的赋值或映射。变量可以取什么值是由我们将要称之为数据逻辑的内容单独定义的；在第二部分我们已经看到，在TLA+的数据逻辑中，一个值是ZFC集合论的任何一个集合。尽管行为总是无限的，我们称达到某个前缀中的终止状态之后再也不改变的行为为终止行为。一个抽象系统是行为的集合。这就是Lamport所说的标准模型。
