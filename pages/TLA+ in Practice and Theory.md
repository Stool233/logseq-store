		...
		- ### 逻辑连接词与量词
			...
			- 因为它被解析为
				- $$
				  ...
				  $$
			- 其中 \( a \), \( b \), \( c \), 和 \( d \) 代表任意子公式。注意开头的连词表示析取和合取列表的开始。隐含的括号从开头连词之后的表达式开始，一直延伸到在某新行中遇到相同类型的其他连词为止。因此，上述内容与以下内容相同（TLA+ 也允许使用以下格式）：
			- $$
			  (a) \lor ((b \land c) \land (d)) \lor (e)
			  $$
			- 因此，以下表达式是格式正确的（尽管变量 \( x \) 被两个不同的量词绑定），
			- $$
			  \lor \forall x : P(x) \lor \exists x : Q(x)
			  $$
			- 因为它被解析为：
			- $$
			  (\forall x : P(x)) \lor (\exists x : Q(x))
