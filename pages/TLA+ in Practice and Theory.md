...
- https://pron.github.io/posts/tlaplus_part4
	...
	- ## **组合**
		...
		- ### 基于模型的规范和信息隐藏
			...
			- 请注意，在 $Stack \triangleq \exists \text{buf}: \text{Internal(buf)} \land \text{IStack}$ 中隐藏 $\text{buf}$ 必须在一个不同于 $\text{InternalStack}$ 的模块中完成，一个不在 $\text{VARIABLES}$ 子句中将 $\text{buf}$ 声明为自由变量的模块中，因为 TLA+ 不允许重新定义任何名称。
		- **开放系统规范**
