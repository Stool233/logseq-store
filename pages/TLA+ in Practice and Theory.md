...
- https://pron.github.io/posts/tlaplus_part3
	...
	- ## **标准模型、安全性与活跃性**
		...
		- ### 两个严重问题
			...
			- $$
			  \land x \in \text{Nat} \land t = 0 \land \Box(t' = t + 1 \land x' = x + 10 \land t = 10 \Rightarrow x = 100)
			  $$
			- 这就是“边界条件”。
			- 或者，如果你更喜欢，我们可以使用等价关系 \(\Box(A \land B) \equiv \Box A \land \Box B\) 来重写公式：
			- $$
			  \land t = 0 \land x \in \text{Nat} \land \Box(t' = t + 1 \land x' = x + 10) \land \Box(t = 10 \Rightarrow x = 100)
			  $$
			- 这也是“边界条件”。
	...
	- ## **停顿不变性和机器闭包**
		...
		- ### 在停顿下的不变性
			...
			- ```
			  VARIABLES h, m
			  HourClock ≜ h ∈ 1..12 ∧ □[h′ = IF h = 12 THEN 1 ELSE h + 1]h ∧ □⋄⟨h′ ≠ h⟩h
			  Clock ≜ h ∈ 1..12 ∧ m ∈ 0..59 ∧ □[m′ = (m + 1) % 60 ∧ m < 59 ⇒ UNCHANGED h ∧ m = 59 ⇒ h′ = IF h = 12 THEN 1 ELSE h + 1]⟨h, m⟩ ∧ □⋄⟨h′ ≠ h⟩h ∧ □⋄⟨m′ ≠ m⟩m
			  ```
			- 确实，现在 `Clock` ⇒ `HourClock`（稍后我们将看到如何使用语法操作来展示类似的事实）。
			- 这里的连词（□⋄(...)）的确切意义将在稍后解释（我们还将看到一个写它们的快捷方式），但由于 □[A]e 可能意味着动作 A 从未被采取（即，初始状态永远停顿），这些连词确保了“活性”属性，即时钟将无限次地继续滴答作响。
			-
-