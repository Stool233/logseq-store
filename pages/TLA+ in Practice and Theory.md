...
- https://pron.github.io/posts/tlaplus_part3
	...
	- ## **状态机**
		...
		- ### 更复杂的示例
			...
			- 这个规格说明使用了两种不同的“忽略细节”的形式。一种是在选择要排序的区域和特定分区时使用不确定性；另一种是将分区操作作为数学定义用数据逻辑而非计算来指定。
			- 很容易看出不确定性是一种精确的细节缺失。例如，因为 \( x' = 1 \Rightarrow x' \in \{0, 1\} \)，显然 \((x = 0 \land \Box [x' = 1] x) \Rightarrow (x = 0 \land \Box [x' \in \{0, 1\}] x)\)。但是，使用数学定义而非计算来“忽略细节”在何种精确意义上呢？我们将在第四部分回答这个问题。
			-
			-
			-
		-
