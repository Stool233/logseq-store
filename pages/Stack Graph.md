...
- Stack graphs（Name resolution at scale）
	...
	- 5. Discussion
		...
		- 一个 LSP 服务器通常作为一个交互式的“sidecar”进程与本地编辑器一起运行，回答由编辑器中的用户交互触发的查询请求。这种设计使得 LSP 服务器不适合大型软件工厂，因为它需要维护足够大的 LSP 服务器群来处理预期的最大并发用户查询数量。此外，我们还必须为每种支持的语言维护单独的 LSP 服务器群。维护这些群的操作负担抵消了复用现有 LSP 实现节省的开发时间。
			-
			-