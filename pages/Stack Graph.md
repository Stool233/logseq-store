...
- Stack graphs（Name resolution at scale）
	...
	- 4 Optimizing queries using partial paths(使用部分路径优化查询)
		...
		- 部分路径为我们在索引时间和查询时间之间的工作提供了更好的平衡。
			...
			- 在查询时间，我们的算法的总体结构与之前相同。我们跟踪待处理的部分路径，并将它们与兼容的部分路径扩展连接起来，而不是跟踪待处理的路径并向它们追加兼容的边。这个过程保证找到所有满足§3.8路径构造判断的名称绑定路径。像我们以前的算法一样，我们将堆栈图的构造成本在多个查询中摊销，通过在索引时间执行一次。通过在索引时间预计算部分路径，新的算法也摊销了路径查找搜索的大部分。
		-