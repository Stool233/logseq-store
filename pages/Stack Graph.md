		...
		- 一些说明
			- 我们还展示了正确解析引用x9到其定义x3的名称绑定路径。
				- 这需要通过导入边解析几个中间引用：
					...
					- A7到其超类；
					- a5到定义该超类的模块。
		- 重要的是，nsamron范围图可以为每个文件划分为不相交的子图，用阴影表示。除了根节点(它是跨所有文件共享的单例节点)，每个节点都属于某个文件的子图。每条边连接属于同一文件的两个节点，或者连接一个节点到共享根节点。在查询时，我们必须将范围图作为一个整体来考虑，因为名称绑定路径可以从一个子图交叉到另一个子图。然而，我们可以在索引时构造每个文件的子图，而不必考虑程序中任何其他文件的内容。因此，nsamron作用域图的构建正是我们所需要的文件增量式的。
		-