...
- Stack graphs（Name resolution at scale）
	...
	- 2.2 Van Antwerpen scope graphs
		...
		- 我们使用方头边将每个作用域连接到该作用域中的定义。
			- 这些边被标记以指示定义的种类：a : 边指定在作用域中定义的符号的类型，而a () 边指定在作用域中定义的可调用符号的返回类型。对于Python类，我们需要两个标签的边，因为它们既可以通过名称引用（得到类本身），也可以被调用（得到类的实例）。同样，我们为每个类有两个关联的作用域：一个（20和40）用于类成员，一个（21和41）用于实例成员。每对作用域通过一条边连接，模拟类成员也作为实例成员可用的方式。
			-
			-
			-