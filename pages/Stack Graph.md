...
- Stack graphs（Name resolution at scale）
	...
	- 5. Discussion
		...
		- 反之，堆栈图可以通过纯粹的语法过程生成，因为所有的名称绑定逻辑都被编码在结果图结构中。为了支持这一点，我们创建了一个新的声明性图构建语言，称为 tree-sitter-graph [16]。基于 tree-sitter 解析框架 [1]，语言专家定义了匹配语言语法的模式，并且应为源文件的具体语法树中的那些模式的每个实例创建哪些堆栈图节点和边的“装置”。这些模式对于每种语言只定义一次。因此，构建堆栈图不需要项目所有者的任何配置，并且不需要调用特定于项目的，通常较慢的，构建过程。
			-