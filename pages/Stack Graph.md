...
- Stack graphs（Name resolution at scale）
	...
	- 2.2 Van Antwerpen scope graphs
		...
		- 虽然我们已经获得了执行类型依赖查找的能力，但我们失去了文件增量性。
			- 积极的中间查找产生的边跨越了文件边界，违反了Néron作用域图的不相交性质。这种违规并不是表面上的；它强调每个中间查找可能依赖于程序中的任何其他文件。我们不能完全单独地分析每个文件，因为我们无法预先知道我们可能需要检查哪些其他文件。更糟糕的是，对其他文件的未来更新可能会使我们创建的子图失效。在病态情况下，对一个文件的更改可能需要我们为整个程序重新生成作用域图。
			-
			-
			-