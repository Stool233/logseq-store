...
- Stack graphs（Name resolution at scale）
	...
	- 5. Discussion
		...
		- Zwaan [18] 展示了如何为 van Antwerpen 的作用域图添加增量差异。
			...
			- 这意味着我们必须为文件版本出现在每次提交中的每个文件的作用域子图存储单独的副本。由此产生的存储成本将过于高昂。相比之下，因为堆栈图内容是文件增量的，每个文件的子图可以单独存储一次，并在文件版本在项目历史中出现的任何时候重用。此外，我们可以早期检测可以跳过的预计算工作，只使用 git 提供的 blob 标识符。
		-
		-