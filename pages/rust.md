...
-
- 在 Rust 中，`dyn` 关键字是动态分发的简写，用于表示后面跟着的类型是一个动态分发的 trait 对象。
	- Trait 对象允许多态，即不同类型的值可以被统一处理，只要它们实现了某个特定的 trait。
	- `dyn` 关键字的引入是为了在语法上明确区分动态分发和静态分发，并提高代码的可读性。
-
- 智能指针
	...
	- `Box<T>`
		...
		- `Pin<T>` 是一个包装器类型，用于保证包装内的数据的内存地址不会改变。
			- 这意味着，如果你有一个 `Pin<Box<T>>`，T 类型的值不能被移动出 `Box`。
			- 这对于异步编程来说非常重要，因为异步任务可能在多次轮询调用之间保持挂起状态，要求底层数据保持稳定。
-
